...
class TokenPoseLandmarker:
    def __init__(self, device="cpu"):
        ...
        args = {
            'det_config': "./creadto-model/detection/cascade/cascade_rcnn_x101_64x4d_fpn_coco.py",
            'det_checkpoint': "./creadto-model/detection/cascade/cascade_rcnn_x101_64x4d_fpn_20e_coco.pth",
    ...
    def __call__(self, images=None, filenames=None):
        ...
        for image in images:
            ...
            pose_result, info = self.func['top_down'](
                ...
                outputs=None)
            stub = dict()
            stub['keypoints'] = torch.stack([torch.tensor(x['keypoints'], dtype=torch.float32) for x in pose_result]).to(self.device)
            stub['scores'] = torch.stack([torch.tensor(x['bbox'][-1], dtype=torch.float32) for x in pose_result]).to(self.device)
            stub['boxes'] = torch.stack([torch.tensor(x['bbox'][:4], dtype=torch.float32) for x in pose_result]).to(self.device)
            center_x = (stub['boxes'][:, 2] + stub['boxes'][:, 0]) / 2.
            center_y = (stub['boxes'][:, 3] + stub['boxes'][:, 1]) / 2.
            stub['center'] = torch.cat([center_x, center_y]).to(self.device)
            stub['info'] = info
            result.append(stub)
        return result
