...
from creadto.utils import log
from creadto.utils import dist
from creadto.trainer.base import Base
from creadto.trainer.utils.precision import MixedPrecisionTrainer
from creadto.trainer.utils.diffusion.resample import LossAwareSampler, create_named_schedule_sampler
...
class DiffusionLoop(Base):
    def __init__(self, args, train_platform, model, diffusion, dataloader, device,
                 use_ema=True, use_fp16=True, fp16_scale_growth=1e-4):
        self.args = args
        ...
        self.model_for_eval = self.model_avg if use_ema else self.model
    ...
    def _load_and_sync_parameters(self):
        ...
        if resume_checkpoint:
            ...
            log.log(f"loading model from checkpoint: {resume_checkpoint}...")
            state_dict = dist.load_state_dict(
                resume_checkpoint, map_location=dist.device())
    ...
    def _load_optimizer_state(self):
        ...
        if bf.exists(opt_checkpoint):
            ...
            state_dict = dist.load_state_dict(
                opt_checkpoint, map_location=dist.device()
            )
    ...
    def _run_train_epoch(self, index, progress):
        progress = super()._run_train_epoch(index, progress, override=True)
        if self.args.use_ema:
            self.model_avg.train()
                        
        for motion, cond in tqdm(progress):
            ...
            if self.total_step() % self.log_interval == 0:
                for k,v in log.dump_dictionary().items():
                    if k == 'loss':
    ...
    def loop(self):
        return super(DiffusionLoop, self).loop()
    ...
    def forward_backward(self, batch, cond):
        ...
        for i in range(0, batch.shape[0], self.microbatch):
            ...
            t, weights = self.schedule_sampler.sample(micro.shape[0], dist.device())
    ...
    def log_step(self):
        log.log_keyvalue("step", self.total_step())
        log.log_keyvalue("samples", (self.total_step() + 1) * self.global_batch)
    ...
    def find_resume_checkpoint(self) -> Optional[str]:
        '''look for all file in save directory in the patent of model{number}.pt
        ...
        TODO: This means ignoring the flag of resume_checkpoint in case some other checkpoints exists in that dir!
        '''
        ...
        return osp.join(self.args.save_dir, models[max(models)]) if models else None
...
def get_blob_logdir():
    ...
    # a blob store or some external drive.
    return log.get_dir()



def log_loss_dict(diffusion, time_step, losses):
    for key, values in losses.items():
        log.log_keyvalue_mean(key, values.mean().item())
        # Log the quantiles (four quartiles, in particular).
        for sub_t, sub_loss in zip(time_step.cpu().numpy(), values.detach().cpu().numpy()):
            quartile = int(4 * sub_t / diffusion.num_timesteps)
            log.log_keyvalue_mean(f"{key}_q{quartile}", sub_loss)