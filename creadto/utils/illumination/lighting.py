...
class PointLight(nn.Module):
    ...
    def __init__(self, ambient_color=np.array([1.0, 1.0, 1.0],), diffuse_color=np.array([1.0, 1.0, 1.0]),
                 specular_color=np.array([1.0, 1.0, 1.0]), location=np.array([1, 1, 8],)) -> None:
        """
            Args:
        ...
        self.ambient_color = torch.from_numpy(ambient_color)[None, ...]
        self.diffuse_color = torch.from_numpy(diffuse_color)[None, ...]
        self.specular_color = torch.from_numpy(specular_color)[None, ...]
        self.location = torch.from_numpy(location)[None, ...]
...
def diffuse(normals, color, direction) -> torch.Tensor:
    ...
    normals, color, direction = convert_to_tensors_and_broadcast(normals, color, direction)
...
def specular(points, normals, direction, color, camera_position, shininess) -> torch.Tensor:
    """
    ...
    matched_tensors = convert_to_tensors_and_broadcast(points, color, direction, camera_position, shininess)
    ...
    # Renormalize the normals in case they have been interpolated.
    # We tried a version that uses F.cosine_similarity instead of renormalizing, but it was slower.
    normals = F.normalize(normals, p=2, dim=-1, eps=1e-6)
    direction = F.normalize(direction, p=2, dim=-1, eps=1e-6)
